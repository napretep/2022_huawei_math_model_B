...
class Statistics:
    ...
    def calcUseRate(self):
        sum(b.freeArea() for b in self.boards) / (H * W * len(self.boards))
...
if __name__ == "__main__":
    for i in range(1,6):# [item_order,item_material,item_id,max_length,min_length, series_id]

        items = Utils.readCsv(datapathB(i))
        series=Utils.readCsv(seriesPathB(i)).astype(np.int32)
        items = np.insert(items,0,series[:,1][items[:,0]],axis=1)
        allStats= []
        allTable = []
        seriesNames = set(items[:,0])
        for seriesName in seriesNames:
            sameSeriesItems = items[[i[0]==seriesName for i in items]]
            materialNames = set(sameSeriesItems[:,1])
            for materialName in materialNames:
                sameMaterialItems = items[[i[1]==materialName for i in sameSeriesItems]]
